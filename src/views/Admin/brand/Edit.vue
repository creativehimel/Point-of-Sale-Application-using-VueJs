...
onBeforeMount(async () => {
  await brand.getBrandById(id.value)
})
...
const onSubmit = handleSubmit((id, values) => {
  //console.log(values)
  brand.handleUpdateBrand(id, values)
})
</script>
<template>
  <DashboardLayout>
    ...
    <div>
      <form @submit="onSubmit(band.singleBand.id)" enctype="multipart/form-data">
        <Card class="overflow-y-auto rounded">
          ...
          <CardContent class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 -mt-2">
            <FormField
              ...
              :model-value="brand.singleBand.name"
              name="name"
            ...
            <div class="md:col-span-2">
              <FormField
                v-slot="{ componentField }"
                :model-value="brand.singleBand.description"
                name="description"
                :validate-on-blur="!isFieldDirty"
              >
                <FormItem>
          ...
          <CardFooter class="flex-col space-y-2">
            <Button v-if="!brand.isLoading" class="w-full mb-2.5" type="submit">Update</Button>
